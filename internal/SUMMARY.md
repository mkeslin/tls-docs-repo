# Table of contents

* [Home](README.md)

## Documentation meta

* [Documentation meta](documentation/README.md)
  * [Behavior contract flywheel](documentation/behavior-contract-flywheel.md)
  * [Pilot: Court PRE-PLEA scenarios](documentation/scenarios/court-preplea-pilot.md)
  * [Gap analysis: Court / Accounting / Collections](documentation/scenarios/gap-analysis-court-accounting-collections.md)
  * [Gap analysis: Jail](documentation/scenarios/gap-analysis-jail.md)
  * [Repository alignment plan](documentation/repository-alignment-plan.md)
  * [Content inventory](documentation/content-inventory.md)

## Strategy

* [Strategy](strategy/README.md)
  * [Vision and principles](strategy/vision-and-principles.md)
  * [Strategic roadmap 2026–2029](strategy/strategic-roadmap-2026-2029.md)
  * [Annual operating plan — 2026 H2](strategy/annual-operating-plan-2026-h2.md)
  * [2026 H2 monthly product releases](strategy/2026-h2-monthly-product-releases.md)
  * [Go-to-market](strategy/go-to-market.md)
  * [Quarterly scorecard](strategy/quarterly-scorecard.md)
  * [Epic plan goals](strategy/epic-kpis.md)

## Company Operating System

* [Business Systems Architecture](operating-system/business-systems-architecture.md)

## Customer Value Engine

* [Customer Value Engine](customer-value-engine/README.md)
  * [Overview](customer-value-engine/overview.md)
  * [Customer value stream](customer-value-engine/customer-value-stream.md)
  * [Acquire](customer-value-engine/acquire/README.md)
    * [Awareness](customer-value-engine/acquire/awareness.md)
    * [Cold leads](customer-value-engine/acquire/cold-leads.md)
    * [Warm leads](customer-value-engine/acquire/warm-leads.md)
    * [Hot leads](customer-value-engine/acquire/hot-leads.md)
    * [Demo](customer-value-engine/acquire/demo.md)
    * [Proposal](customer-value-engine/acquire/proposal.md)
    * [Contract](customer-value-engine/acquire/contract.md)
    * [Invoice](customer-value-engine/acquire/invoice.md)
  * [Deliver](customer-value-engine/deliver/README.md)
    * [Sales handoff](customer-value-engine/deliver/sales-handoff.md)
    * [Kickoff and discovery](customer-value-engine/deliver/kickoff-and-discovery.md)
    * [Infrastructure](customer-value-engine/deliver/infrastructure.md)
    * [Data migration](customer-value-engine/deliver/data-migration.md)
    * [Configuration](customer-value-engine/deliver/configuration.md)
    * [Integrations and hardware](customer-value-engine/deliver/integrations-and-hardware.md)
    * [Training](customer-value-engine/deliver/training.md)
    * [Go live](customer-value-engine/deliver/go-live.md)
    * [Hypercare and transition](customer-value-engine/deliver/hypercare-and-transition.md)
  * [Operate](customer-value-engine/operate/README.md)
    * [Support](customer-value-engine/operate/support.md)
    * [Product updates](customer-value-engine/operate/product-updates.md)
  * [Expand](customer-value-engine/expand/README.md)
    * [Customer success](customer-value-engine/expand/customer-success.md)
    * [Expansion](customer-value-engine/expand/expansion.md)
  * [Advocate](customer-value-engine/advocate/README.md)
    * [Reference](customer-value-engine/advocate/reference.md)
    * [Referral](customer-value-engine/advocate/referral.md)
    * [Community](customer-value-engine/advocate/community.md)

## SOPs

* [Standard operating procedures (SOPs)](sops/README.md)
  * [SOPs — Acquire](sops/acquire.md)
    * [Generate new leads](sops/acquire/generate-new-leads.md)
    * [Run product demo](sops/acquire/run-product-demo.md)
    * [Prepare proposal](sops/acquire/prepare-proposal.md)
    * [Execute contract](sops/acquire/execute-contract.md)
  * [Deliver](sops/deliver/README.md)
    * [Implementation overview](sops/deliver/implementation-overview.md)
    * [Creating a new implementation](sops/deliver/creating-a-new-implementation.md)
    * [Implementation lifecycle](sops/deliver/implementation-lifecycle.md)
    * [Implementation workspace standard](sops/deliver/implementation-workspace-standard.md)
    * [Implementation roles and responsibilities](sops/deliver/implementation-roles-and-responsibilities.md)
    * [Legacy onboarding questionnaire mapping](sops/deliver/legacy-onboarding-questionnaire-mapping.md)
    * [Phase 0 - Sales handoff](sops/deliver/sales-handoff.md)
    * [Phase 1 - Kickoff and discovery](sops/deliver/kickoff-and-discovery.md)
    * [Phase 2 - Infrastructure](sops/deliver/infrastructure/README.md)
      * [Bootstrap environment](sops/deliver/infrastructure/bootstrap-environment.md)
      * [Bootstrap environment standard](sops/deliver/infrastructure/bootstrap-environment-standard.md)
      * [Environment inventory standard](sops/deliver/infrastructure/environment-inventory-standard.md)
      * [Environment lifecycle](sops/deliver/infrastructure/environment-lifecycle.md)
      * [Environment classification](sops/deliver/infrastructure/environment-classification.md)
      * [Baseline database standard](sops/deliver/infrastructure/baseline-database-standard.md)
      * [Bootstrap vs configuration](sops/deliver/infrastructure/bootstrap-vs-configuration.md)
      * [Hub environment integration](sops/deliver/infrastructure/hub-environment-integration.md)
    * [Phase 3 - Data migration](sops/deliver/data-migration/README.md)
      * [Migration philosophy](sops/deliver/data-migration/migration-philosophy.md)
      * [Vendor packages](sops/deliver/data-migration/vendor-packages/README.md)
        * [Migration package standards](sops/deliver/data-migration/vendor-packages/migration-package-standards.md)
        * [Vendor conversion guides](sops/deliver/data-migration/vendor-packages/vendor-conversion-guides/README.md)
          * [CrimeStar](sops/deliver/data-migration/vendor-packages/vendor-conversion-guides/crimestar.md)
          * [CopSync / Kologik](sops/deliver/data-migration/vendor-packages/vendor-conversion-guides/copsync-kologik.md)
          * [Cardinal Badge](sops/deliver/data-migration/vendor-packages/vendor-conversion-guides/cardinal-badge.md)
          * [Tyler](sops/deliver/data-migration/vendor-packages/vendor-conversion-guides/tyler.md)
          * [FundView](sops/deliver/data-migration/vendor-packages/vendor-conversion-guides/fundview.md)
          * [Asyst](sops/deliver/data-migration/vendor-packages/vendor-conversion-guides/asyst.md)
          * [Xpediter](sops/deliver/data-migration/vendor-packages/vendor-conversion-guides/xpediter.md)
      * [Validation](sops/deliver/data-migration/migration-validation-standard.md)
      * [Customer acceptance](sops/deliver/data-migration/customer-acceptance.md)
      * [Legacy system migration](sops/deliver/data-migration/legacy-system-migration.md)
      * [Post-conversion utilities](sops/deliver/data-migration/post-conversion-utilities.md)
      * [Migration architecture](sops/deliver/data-migration/migration-architecture.md)
      * [Migration decision matrix](sops/deliver/data-migration/migration-decision-matrix.md)
      * [Migration overrides & mapping standard](sops/deliver/data-migration/migration-customer-configuration.md)
      * [Migration metrics](sops/deliver/data-migration/migration-metrics.md)
    * [Phase 4 - Configuration](sops/deliver/configuration/README.md)
      * [Agency & module settings](sops/deliver/configuration/agency-settings.md)
    * [Phase 5 - Integrations and hardware](sops/deliver/integrations-and-hardware.md)
    * [Phase 6 - Training](sops/deliver/training.md)
    * [Phase 7 - Go live](sops/deliver/go-live.md)
      * [Go-live runbook](sops/deliver/go-live-runbook.md)
    * [Phase 8 - Hypercare and transition](sops/deliver/hypercare-and-transition.md)
  * [SOPs — Operate](sops/operate.md)
    * [Triage support request](sops/operate/triage-support-request.md)
    * [Publish product update](sops/operate/publish-product-update.md)
    * [Enable soft-launch RMS module](sops/operate/enable-soft-launch-rms-module.md)
  * [SOPs — Expand](sops/expand.md)
    * [Customer success check-in](sops/expand/customer-success-check-in.md)
    * [Expand account](sops/expand/expand-account.md)
    * [Deliver → Expand handoff](sops/expand/deliver-to-expand-handoff.md)
  * [SOPs — Advocate](sops/advocate.md)
    * [Capture referral](sops/advocate/capture-referral.md)
    * [Request customer reference](sops/advocate/request-customer-reference.md)
    * [Reference champion refresh](sops/advocate/reference-champion-refresh.md)
    * [Community event follow-up](sops/advocate/community-event-follow-up.md)

## Assessments

* [Assessments](assessments/README.md)
  * [Legacy System Migration Assessment](assessments/legacy-system-migration-assessment.md)
  * [Implementation Readiness Assessment](assessments/implementation-readiness-assessment.md)
  * [Go-Live Readiness Assessment](assessments/go-live-readiness-assessment.md)

## Compliance

* [Compliance](compliance/README.md)
  * [TLETS application (internal)](compliance/tlets/README.md)
    * [Word forms (download)](compliance/tlets/forms/README.md)
      * [TLETS Interface Questionnaire (.docx)](compliance/tlets/forms/tlets-interface-questionnaire.docx)
      * [CJIS Interface Approval Packet (.docx)](compliance/tlets/forms/cjis-security-office-interface-approval-packet.docx)
      * [Agency remote access policy (.docx)](compliance/tlets/forms/agency-remote-access-policy-template.docx)
      * [Thin Line remote access addendum (.docx)](compliance/tlets/forms/thin-line-remote-access-addendum-template.docx)
      * [Agency boundary equipment worksheet (.csv)](compliance/tlets/forms/agency-boundary-equipment-worksheet.csv)
      * [Cloud data storage disclosure (.docx)](compliance/tlets/forms/cloud-data-storage-disclosure-template.docx)
      * [SIEM monitoring and weekly review (.docx)](compliance/tlets/forms/si-4-siem-monitoring-and-weekly-review.docx)
      * [SIEM weekly review log (.csv)](compliance/tlets/forms/si-4-weekly-review-log.csv)
    * [Interface Questionnaire answers](compliance/tlets/interface-questionnaire.md)
    * [Interface Approval Packet answers](compliance/tlets/interface-approval-packet.md)
    * [Agency remote access policy template](compliance/tlets/agency-remote-access-policy-template.md)
    * [Thin Line remote access addendum](compliance/tlets/thin-line-remote-access-addendum-template.md)
    * [Agency network diagram template](compliance/tlets/agency-network-diagram-template.md)
    * [Thin Line network diagram insert](compliance/tlets/thin-line-network-diagram-insert.md)
    * [Agency boundary equipment worksheet](compliance/tlets/agency-boundary-equipment-worksheet.md)
    * [Cloud data storage disclosure](compliance/tlets/cloud-data-storage-disclosure-template.md)
    * [SIEM monitoring and weekly review](compliance/tlets/si-4-siem-monitoring-and-weekly-review.md)
    * [Direct-interface scope](compliance/tlets/direct-interface-scope.md)
    * [Open decisions](compliance/tlets/open-decisions.md)
    * [Agency guide (planned)](compliance/tlets/agency-guide-planned.md)

## Backlog & design

* [Backlog & design](backlog/README.md)
  * [How we capture work](backlog/how-we-capture-work.md)
  * [Prioritized backlog](backlog/prioritized.md)
  * [Plans](backlog/plans/README.md)
    * [BL-001 OCA February verify](backlog/plans/BL-001-oca-february-verify.md)
    * [BL-002 Jail booking verify](backlog/plans/BL-002-jail-booking-verify.md)
    * [BL-003 Jail command center verify](backlog/plans/BL-003-jcc-verify.md)
    * [BL-014 Criminal trespass](backlog/plans/BL-014-criminal-trespass.md)
    * [BL-015 Accounting grid prints](backlog/plans/BL-015-accounting-grid-prints.md)
    * [BL-018 Warrant project testing](backlog/plans/BL-018-warrant-project-testing.md)
    * [BL-019 Shallowater Municipal Court go-live](backlog/plans/BL-019-shallowater-municipal-court-golive.md)
    * [BL-020 CJIS logout cookie revocation](backlog/plans/BL-020-cjis-logout-cookie-revocation.md)
    * [BL-021 TlsPage Classic shell](backlog/plans/BL-021-tls-page-shell-classic-layout.md)
    * [BL-022 CAD large-agency remediation](backlog/plans/BL-022-cad-large-agency-remediation.md)
    * [BL-022 CAD state centralization](backlog/plans/BL-022-cad-state-centralization.md)
    * [BL-022-12 Call templates](backlog/plans/BL-022-12-call-templates.md)
    * [BL-023 Collections supplemental item referrals](backlog/plans/BL-022-collections-supplemental-item-referrals.md)
    * [CAD pinned closed calls](backlog/plans/CAD-follow-up-pinned-closed-calls.md)
    * [FB-13 Scheduled calls](backlog/plans/FB-13-scheduled-calls.md)
    * [Collections module](backlog/plans/collections-module.md)
    * [Court third-party collections](backlog/plans/court-violation-third-party-collections.md)
    * [Court VPTA separate charge](backlog/plans/court-vpta-separate-charge.md)
    * [Equipment and fleet modules](backlog/plans/equipment-fleet-modules.md)
    * [Jail intake arresting agency](backlog/plans/jail-intake-arresting-agency-three-mode.md)
    * [JLM observation pass milestones](backlog/plans/jlm-observation-pass-milestones.md)
    * [Mental health screening notification rules](backlog/plans/mental-health-screening-notification-rules.md)
    * [Hub / Directory source-of-truth migration](backlog/plans/hub-directory-source-of-truth-migration.md)
  * [Design documents](backlog/design/README.md)
    * [Design doc template](backlog/design/design-doc-template.md)
    * [Patrol officer command center](backlog/design/patrol-officer-command-center.md)
    * [Levelland fit — Patrol + CAD 2026 roadmap](backlog/design/levelland-patrol-cad-2026-roadmap.md)
    * [Investigation interface](backlog/design/investigation-interface.md)
    * [Supervisor command center](backlog/design/supervisor-command-center.md)
    * [Data integrity interface](backlog/design/data-integrity-interface.md)
    * [Security command center](backlog/design/security-command-center.md)
    * [Hub / Directory tenant–agency ownership](backlog/design/hub-directory-tenant-agency-ownership.md)
    * [Hub Acquire Desk](backlog/design/hub-acquire-desk.md)
    * [Hub notecard board](backlog/design/hub-notecard-board.md)
    * [Hub Quo tasks overlay](backlog/design/hub-quo-tasks.md)
    * [Hub Outlook / Teams calendar overlay](backlog/design/hub-outlook-calendar.md)
    * [Hub Pipedrive overlay](backlog/design/hub-pipedrive-overlay.md)
    * [Hub epic plans](backlog/design/hub-epic-plans.md)
    * [Hub Deliver Desk](backlog/design/hub-deliver-desk.md)
    * [Hub Operate Desk](backlog/design/hub-operate-desk.md)
    * [Hub Expand Desk](backlog/design/hub-expand-desk.md)
    * [Hub Advocate Desk](backlog/design/hub-advocate-desk.md)
    * [Hub Internal Desk](backlog/design/hub-internal-desk.md)
  * [Feedback](backlog/feedback/README.md)
    * [Crosby Jail site visit 2026-06-18](backlog/feedback/2026-06-18-crosby-jail-site-visit.md)
  * [Plan template](backlog/templates/plan-template.md)

## Playbooks and references

* [Implementation](implementation/README.md)
* [Customer success](customer-success.md)
* [Product](product.md)
* [Technical](technical.md)
  * [Codebase size by release](technical/codebase-size-by-release.md)
* [Policies](policies.md)
  * [Migration Pricing Policy](policies/migration-pricing.md)
  * [Acquire authority matrix](policies/acquire-authority.md)
  * [Operate authority matrix](policies/operate-authority.md)
  * [SaaS pricing and discount guardrails](policies/saas-pricing-and-discount-guardrails.md)
* [Checklists](checklists/README.md)
  * [Demo checklist](checklists/demo-checklist.md)
  * [Sales Handoff Checklist](checklists/sales-handoff-checklist.md)
  * [Environment Health Checklist](checklists/environment-health-checklist.md)
  * [Agency Configuration Checklist](checklists/agency-configuration-checklist.md)
  * [Hardware Readiness Checklist](checklists/hardware-readiness-checklist.md)
  * [Customer Validation Checklist](checklists/customer-validation-checklist.md)
  * [Migration Close-Out Checklist](checklists/migration-close-out-checklist.md)
  * [Account health / risk checklist](checklists/account-health-risk-checklist.md)
  * [Customer success check-in checklist](checklists/customer-success-check-in-checklist.md)
  * [Expansion conversation checklist](checklists/expansion-conversation-checklist.md)
  * [Deliver → Expand handoff checklist](checklists/deliver-to-expand-handoff-checklist.md)
  * [Capture referral checklist](checklists/capture-referral-checklist.md)
  * [Reference agency qualification checklist](checklists/reference-agency-qualification-checklist.md)
  * [Request customer reference checklist](checklists/request-customer-reference-checklist.md)
  * [Reference champion refresh checklist](checklists/reference-champion-refresh-checklist.md)
  * [Community event follow-up checklist](checklists/community-event-follow-up-checklist.md)
* [Templates](templates.md)
  * [Field-day stop list](templates/field-day-stop-list.md)
  * [Referral ask script](templates/referral-ask.md)
  * [First-touch outreach](templates/first-touch-outreach.md)
  * [Customer release email](templates/customer-release-email.md)
  * [Two-week sales plan](templates/sales-plan-two-week.md)
  * [Working session workbook](templates/working-session-workbook.md)
  * [Go-live decision record](templates/go-live-decision-record.md)
  * [Conversion Summary](templates/conversion-summary.md)
  * [Customer Acceptance Form](templates/customer-acceptance-form.md)
