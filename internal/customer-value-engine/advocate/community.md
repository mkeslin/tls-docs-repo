# Community

**Phase:** Advocate  
**Desired outcome:** Strengthen ecosystem

## Inputs

Engaged customers

## Outputs

Ecosystem and product feedback

## Owner

Fugate

## Current process

Conferences

## Tooling

Email, events, newsletter

## Capacity (today)

Unlimited

## Cycle time

Continuous

## Maturity

**1 / 5 — Founder-driven**

| Score | Meaning |
|------:|---------|
| 1 | Founder-driven |
| 2 | Documented |
| 3 | Standardized |
| 4 | Automated |
| 5 | Scalable |

## What would break first?

Doesn't exist

## Continuous improvement (10x ideas)

| Lens | Idea |
|------|------|
| Reduce / Simplify | One-off communication |
| Standardize | Communication calendar |
| Automate | Newsletter, user portal |
| Delegate | Marketing / CS |
| Scale | User groups, annual conference |

## Product responsibility

**TODO / Decision needed:** Not yet assigned in the source Customer Value Engine worksheet.

## Related documents

- Related SOPs: **TODO:** link under `internal/sops/`
- Checklists / templates: **TODO:**