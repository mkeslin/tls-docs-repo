# Contract

**Phase:** Acquire  
**Desired outcome:** Execute agreement

## Inputs

Proposal accepted

## Outputs

Executed agreement

## Owner

Fugate

## Current process

Review, signatures

## Tooling

Dropbox Sign

## Capacity (today)

8–10/mo

## Cycle time

1–14 days

## Maturity

**3 / 5 — Standardized**

| Score | Meaning |
|------:|---------|
| 1 | Founder-driven |
| 2 | Documented |
| 3 | Standardized |
| 4 | Automated |
| 5 | Scalable |

## What would break first?

Customer procurement delays

## Continuous improvement (10x ideas)

| Lens | Idea |
|------|------|
| Reduce / Simplify | Paper processes |
| Standardize | Standard agreements |
| Automate | E-signature workflow |
| Delegate | Sales |
| Scale | Self-service contracting |

## Product responsibility

**TODO / Decision needed:** Not yet assigned in the source Customer Value Engine worksheet.

## Related documents

- Related SOPs: **TODO:** link under `internal/sops/`
- Checklists / templates: **TODO:**