# Sales Handoff

**Phase:** Deliver · lifecycle 0  
**Desired outcome:** Signed customer transferred to Implementation with clear scope

**Phase overview:** [Sales Handoff](../../sops/deliver/sales-handoff.md)

## Inputs

- Signed contract / commercial path
- Scope (modules, migration, integrations, hardware)
- Customer and Sales contacts

## Outputs

- Handoff accepted by Implementation
- Kickoff scheduled
- Risks / open questions listed

## Owner

<mark style="color:red;">**Decision needed:**</mark> Owner not assigned.

## Current process

Package handoff packet → confirm Implementation capacity → schedule kickoff. See [Sales Handoff Checklist](../../checklists/sales-handoff-checklist.md).

## Tooling

CRM / Pipedrive · deal notes

## Capacity (today)

<mark style="color:red;">**TODO:**</mark>

## Cycle time

<mark style="color:red;">**TODO:**</mark>

## Maturity

**1 / 5 — Founder-driven**

## Related documents

- [Sales Handoff](../../sops/deliver/sales-handoff.md)
- [Sales Handoff Checklist](../../checklists/sales-handoff-checklist.md)
- [Migration Pricing Policy](../../policies/migration-pricing.md)
