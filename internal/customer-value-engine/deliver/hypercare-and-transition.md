# Hypercare and Transition

**Phase:** Deliver · lifecycle 8  
**Desired outcome:** Hypercare complete; handoff to Operate / Support

**Phase overview:** [Hypercare and Transition](../../sops/deliver/hypercare.md)

## Inputs

- Customer live in production
- Known issues / deferrals
- Support contacts / SLAs

## Outputs

- Closed critical hypercare issues
- Operate ownership confirmed

## Owner

<mark style="color:red;">**Decision needed:**</mark> Owner not assigned.

## Current process

Run hypercare window → triage → formal transition to [Operate — Support](../operate/support.md).

## Tooling

Support queue · check-in cadence

## Capacity (today)

<mark style="color:red;">**TODO:**</mark>

## Cycle time

<mark style="color:red;">**TODO:**</mark> (typical window length TBD)

## Maturity

**1 / 5 — Founder-driven**

## Related documents

- [Hypercare and Transition](../../sops/deliver/hypercare.md)
- [Operate — Support](../operate/support.md)
